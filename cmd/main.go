...
// cria um novo board e suas colunas padrão
...
// obtém as colunas de um board, garantindo que as colunas de status existam
func (app *App) getColumns(c *fiber.Ctx) error {
	...
	// Verifica e cria a coluna "Solucionado" se não existir
	if !foundSolucionado {
		maxPosition++
	...
	// Verifica e cria a coluna "Não Solucionado" se não existir
	...
	// Se criamos alguma coluna, precisamos buscar a lista completa novamente
	if !foundSolucionado || !foundNaoSolucionado {
		...
		columns = make([]Column, 0) // Limpa a lista para preencher com os dados atualizados
		for rows.Next() {
			var col Column
			if err := rows.Scan(&col.ID, &col.BoardID, &col.Title, &col.Position, &col.Color); err != nil {

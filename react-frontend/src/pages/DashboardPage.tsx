...
const filterCards = (cards: Card[], filters: FilterState): Card[] => {
  return cards.filter((card) => {
    if (
      ...
      card.assigned_to !== filters.colaborador
    ) {
      return false;
    }
    if (
      filters.prioridade !== "todas" &&
      card.priority !== filters.prioridade
    ) {
      return false;
    }
    return true;
...
const MetricCard = ({
  ...
  <div className={styles.dashboardWidget}>
    <div className={styles.metricCardContent}>
      <div className={styles.metricCardText}>
        ...
        {trend && (
          ...
          >
            <i
              className={`fas fa-arrow-${trend.isPositive ? "up" : "down"}`}
            ></i>
            {Math.abs(trend.value)}%
          </p>
        )}
      </div>
      <div className={styles.metricCardIcon} style={iconBackgroundStyle}>
  ...
  return (
    <div className={styles.chartTooltip}>
      ...
      {payload.map((entry: any, index: number) => (
        <p
          key={`item-${index}`}
          style={{ color: entry.stroke || entry.payload.fill || entry.color }}
        >
...
const FilterSection = ({
  ...
  <div className={styles.dashboardFilters}>
    ...
    <div className={styles.dashboardFilterGroup}>
      <label>
        <i className="fas fa-flag"></i>Prioridade
      </label>
      <select
        className={styles.formSelect}
...
const EfficiencyBadge = ({ efficiency }: { efficiency: number }) => {
  const getEfficiencyClass = (eff: number) => {
    if (eff >= 70) return styles.high;
    if (eff >= 50) return styles.medium;
    return styles.low;
  };
  return (
    <span
      className={`${styles.efficiencyBadge} ${getEfficiencyClass(efficiency)}`}
    >
      {efficiency}%
    </span>
  );
...
export function DashboardPage() {
  ...
  const stats = useMemo((): DashboardStats | null => {
    ...
    const generateRealTimelineData = (cards: Card[], numDays: number) => {
      const timeline: Record<
        string,
        { Criados: number; Solucionados: number }
      > = {};
      ...
      cards.forEach((card) => {
        if (card.created_at) {
          ...
          if (
            ...
            if (timeline[formattedCreationDate])
              timeline[formattedCreationDate].Criados++;
          }
        }
        if (card.completed_at && card.column_id === solucionadoId) {
          const completionDate = new Date(card.completed_at);
          if (
            (today.getTime() - completionDate.getTime()) / (1000 * 3600 * 24) <
            numDays
          ) {
      ...
      const currentlySolvedCount = cards.filter(c => c.column_id === solucionadoId).length;
      const currentlyFailedCount = cards.filter(c => c.column_id === naoSolucionadoId).length;
      let pendingCountAtEndOfDay = cards.length - currentlySolvedCount - currentlyFailedCount;

      const timelineEntries = Object.entries(timeline).reverse(); 

      const resultData = timelineEntries.map(([date, values]) => {
          const dataPoint = {
              data: date,
              Criados: values.Criados,
              Solucionados: values.Solucionados,
              Pendentes: Math.max(0, pendingCountAtEndOfDay)
          };
          
          pendingCountAtEndOfDay = pendingCountAtEndOfDay - values.Criados + values.Solucionados;
    ...
    const completed = filteredCards.filter(
      (c) => c.column_id === solucionadoId
    ).length;
    const failed = filteredCards.filter(
      (c) => c.column_id === naoSolucionadoId
    ).length;
    const pending = filteredCards.length - completed - failed;
    const statusData = [
      { name: STATUS_LABELS.pendente, value: pending, fill: PALETTE.pendente },
      {
        name: STATUS_LABELS.solucionado,
        value: completed,
        fill: PALETTE.solucionado,
      },
      {
        name: STATUS_LABELS.naoSolucionado,
        value: failed,
    ...
    const priorityData = Object.entries(PRIORITY_LABELS).map(
      ([key, label]) => ({
        name: label,
    ...
    const colaboradorCounts = filteredCards.reduce((acc, card) => {
      ...
      else if (card.column_id === naoSolucionadoId)
        acc[displayName].naoSolucionados++;
      else acc[displayName].pendentes++;
      return acc;
    }, {} as Record<string, { pendentes: number; solucionados: number; naoSolucionados: number }>);
    const colaboradorData: CollaboratorStats[] = Object.entries(
      colaboradorCounts
    )
      .map(([nome, dados]) => {
        const total =
          dados.pendentes + dados.solucionados + dados.naoSolucionados;
        ...
        return {
          ...
          eficiencia: calculateEfficiency(
            dados.solucionados,
            dados.naoSolucionados
          ),
          produtividade:
            days > 0
              ? Math.round(produtividadeTotal / days)
              : produtividadeTotal,
        };
      })
    ...
    const resolvedCards = filteredCards.filter(
      (c) =>
        (c.column_id === solucionadoId || c.column_id === naoSolucionadoId) &&
        c.created_at &&
        c.updated_at
    );
    ...
    const lastPeriodCards = timelineData
      .slice(-days)
      .reduce((sum, day) => sum + day.Criados, 0);
    const previousPeriodCards = generateRealTimelineData(allCards, days * 2)
      .slice(0, days)
      .reduce((sum, day) => sum + day.Criados, 0);
    const crescimentoSemanal =
      previousPeriodCards > 0
        ? Math.round(
            ((lastPeriodCards - previousPeriodCards) / previousPeriodCards) *
              100
          )
        : lastPeriodCards > 0
        ? 100
    ...
    const produtividade =
      days > 0 ? Math.round(totalResolved / days) : totalResolved;
    const eficienciaMedia =
      colaboradorData.length > 0
        ? Math.round(
            colaboradorData.reduce((sum, c) => sum + c.eficiencia, 0) /
              colaboradorData.length
          )
        : 0;
    return {
      statusData,
  ...
  const colaboradores = useMemo(() => {
    ...
    return uniqueEmails.map((email) => ({
      value: email,
      label: userDisplayNameMap[email] || email,
    }));
  }, [columns]);
    ...
    <div className={styles.dashboardPage}>
      ...
      {isLoading || !stats ? (
        <div className={styles.loadingContainer}>
          <Loader fullScreen={false} text="Analisando dados..." />
        </div>
      ) : (
        <div className={styles.contentFadeIn}>
              ...
              <div className={styles.chartContainer}>
                <ResponsiveContainer width="100%" height="100%">
                  ...
                  >
                    ...
                    />
                    <Bar dataKey="value" name="Tarefas" radius={[8, 8, 0, 0]}>
              ...
              <div className={styles.chartContainer}>
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    ...
                    <Legend
                      wrapperStyle={{ fontSize: "14px", color: PALETTE.texto }}
                    />
                  </PieChart>
                </ResponsiveContainer>
              </div>
          ...
          <div
            className={`${styles.dashboardWidget} ${styles.dashboardTimelineWidget}`}
          >
          ...
          <div
            className={`${styles.dashboardWidget} ${styles.dashboardDetailWidget}`}
          >
            ...
            <div className={styles.tableContainer}>
              <table className={styles.dashboardTable}>
                <thead>
                  <tr>
                    <th className={styles.textLeft}>Colaborador</th>
                    <th>Total</th>
                    <th>Solucionados</th>
                    <th>Pendentes</th>
                ...
                <tbody>
                  {stats.colaboradorData.map((colab, index) => (
                    <tr key={index}>
                      <td className={styles.textLeft}>{colab.name}</td>
                      <td>
                        <strong>{colab.total}</strong>
                      </td>
                      <td
                        style={{
                          color: PALETTE.solucionado,
                      ...
                      <td
                        style={{ color: PALETTE.pendente, fontWeight: "600" }}
                      >
                        {colab.pendentes}
                      </td>
                      <td
                      ...
                      <td>
                        <strong>{colab.produtividade}</strong>
                      </td>
                    </tr>
                  ))}
                  {stats.colaboradorData.length === 0 && (
                    <tr>
                      <td
                        colSpan={7}
                        style={{ textAlign: "center", color: PALETTE.texto }}
                      >
